...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function initAdminDashboard() {
            if ($('.budgetura-admin-dashboard').length) {
                loadDashboardStats();
                loadRecentActivity();
        ...
        function loadDashboardStats() {
            const $container = $('.budgetura-admin-dashboard');
            if (!$container.length) return;

            $.ajax({
                url: budgeturaAdmin.restUrl + 'stats',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgeturaAdmin.restNonce);
                },
        ...
        function loadRecentActivity() {
            ...
            $container.html('<div class="budgetura-loading"><span class="spinner is-active"></span><p>Loading recent activity...</p></div>');

            $.ajax({
                url: budgeturaAdmin.restUrl + 'activity',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgeturaAdmin.restNonce);
                },
        ...
        function applyCreditUtilizationColor(util) {
            const $card = $('#budgetura-credit-util').closest('.budgetura-stat-card');
        ...
        function applyDTIColor(dti) {
            const $card = $('#budgetura-dti').closest('.budgetura-stat-card');
            $card.removeClass('success warning danger');
        ...
        $(document).on('click', '#budgetura-create-snapshot', function(e) {
            ...
            $.ajax({
                url: budgeturaAdmin.restUrl + 'snapshot',
                method: 'POST',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgeturaAdmin.restNonce);
                },
        ...
        function showNotice(message, type = 'info') {
            const $notice = $('<div class="budgetura-notice ' + type + '">' + message + '</div>');
            $('.budgetura-admin-dashboard').prepend($notice);
         ...
         * Form validation
         */
        $('.budgetura-meta-box form').on('submit', function(e) {
            const $form = $(this);
            let isValid = true;

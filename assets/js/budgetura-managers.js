...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function initCreditCardManager() {
            if ($('.budgetura-credit-cards-manager').length) {
                loadCreditCardsList();
                initCreditCardForm();
        ...
        function loadCreditCardsList() {
            const $list = $('#budgetura-credit-cards-list');
            if (!$list.length) return;

            $.ajax({
                url: budgetura.restUrl + 'credit-cards',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
                ...
                error: function(xhr, status, error) {
                    ...
                    $list.html('<div class="budgetura-message error">Failed to load credit cards</div>');
                }
            });
        }
        ...
        function renderCreditCardsList(cards) {
            ...
            cards.forEach(function(card) {
                ...
                html += '<div class="budgetura-card-item" data-id="' + card.id + '" data-status="' + card.status + '">';
                html += '  <div class="budgetura-card-header">';
                html += '    <div>';
                html += '      <h3 class="budgetura-card-name">' + escapeHtml(card.name) + '</h3>';
                html += '      <span class="budgetura-card-badge ' + statusClass + '">' + card.status.replace('_', ' ') + '</span>';
                html += '    </div>';
                html += '    <div class="budgetura-card-actions">';
                html += '      <button class="budgetura-btn budgetura-btn-small" onclick="window.location=\'?action=edit&edit=' + card.id + '\'">' + budgeturaL10n.edit + '</button>';
                html += '      <button class="budgetura-btn budgetura-btn-small budgetura-btn-danger budgetura-delete-card" data-id="' + card.id + '">' + budgeturaL10n.delete + '</button>';
                html += '    </div>';
                html += '  </div>';

                html += '  <div class="budgetura-card-details">';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Balance</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(card.balance) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Limit</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(card.credit_limit) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Utilization</div>';
                html += '      <div class="budgetura-card-detail-value ' + utilClass + '">' + formatPercentage(card.utilization) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">APR</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatPercentage(card.interest_rate) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Monthly Payment</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(card.minimum_payment + card.extra_payment) + '</div>';
                html += '    </div>';
                html += '  </div>';

                // Utilization progress bar
                html += '  <div class="budgetura-progress">';
                html += '    <div class="budgetura-progress-bar ' + utilClass + '" style="width: ' + Math.min(card.utilization, 100) + '%"></div>';
                html += '  </div>';

                // Payoff info
                if (card.status === 'active' && card.months_to_payoff !== Infinity) {
                    html += '  <div class="budgetura-card-payoff">';
                    html += '    <div class="budgetura-payoff-info">';
                    ...
                    html += '  <div class="budgetura-card-payoff warning">';
        ...
        function initCreditCardForm() {
            const $form = $('#budgetura-credit-card-form');
        ...
        function calculatePayoffPreview() {
            ...
            if (balance > 0 && totalPayment > 0) {
                ...
                const utilization = BudgeturaCalculator.calculateUtilization(balance, creditLimit);

                // Calculate payoff
                const months = BudgeturaCalculator.calculateMonthsToPayoff(balance, interestRate, totalPayment);
                const totalInterest = BudgeturaCalculator.calculateTotalInterest(balance, totalPayment, months);
                const payoffDate = BudgeturaCalculator.calculatePayoffDate(months);

                // Update preview
                $('#preview-utilization').text(BudgeturaCalculator.formatPercentage(utilization));
                $('#preview-months').text(months === Infinity ? '∞' : months);
                $('#preview-interest').text(BudgeturaCalculator.formatCurrency(totalInterest));
                $('#preview-date').text(payoffDate);

                $('#budgetura-payoff-preview').slideDown();
            } else {
                $('#budgetura-payoff-preview').slideUp();
            }
        }
        ...
        function saveCreditCard() {
            ...
            const url = budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_credit_card' + (postId > 0 ? '/' + postId : '');
        ...
        function filterAndSortCards() {
            ...
            const $cards = $('.budgetura-card-item');
            ...
            $visibleCards.sort(function(a, b) {
                ...
                if (sortBy.includes('balance')) {
                    aVal = parseFloat($a.find('.budgetura-card-detail-value').first().text().replace(/[^0-9.-]+/g,""));
                    bVal = parseFloat($b.find('.budgetura-card-detail-value').first().text().replace(/[^0-9.-]+/g,""));
                }
         ...
         * Delete credit card
         */
        $(document).on('click', '.budgetura-delete-card', function(e) {
            e.preventDefault();
            ...
            const $cardItem = $(this).closest('.budgetura-card-item');

            $.ajax({
                url: budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_credit_card/' + cardId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
                success: function(response) {
                    $cardItem.fadeOut(function() {
                        ...
                        if ($('.budgetura-card-item').length === 0) {
                            $('#budgetura-credit-cards-list').hide();
                            $('#empty-state').show();
                        }
                    });
        ...
        function showMessage(message, type) {
            const $message = $('<div class="budgetura-message ' + type + '">' + message + '</div>');
            $('.budgetura-dashboard').prepend($message);
...
var budgeturaL10n = budgeturaL10n || {
    edit: 'Edit',
    delete: 'Delete',
    confirm_delete: 'Are you sure you want to delete this item?'
        ...
        function initLoansManager() {
            if ($('.budgetura-loans-manager').length) {
                loadLoansList();
                initLoanForm();
                initLoanControls();
            }
        ...
        function loadLoansList() {
            const $list = $('#budgetura-loans-list');
            if (!$list.length) return;

            $.ajax({
                url: budgetura.restUrl + 'loans',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
                ...
                error: function(xhr, status, error) {
                    ...
                    $list.html('<div class="budgetura-message error">Failed to load loans</div>');
                }
            });
        }
        ...
        function renderLoansList(loans) {
            ...
            loans.forEach(function(loan) {
                html += '<div class="budgetura-card-item" data-id="' + loan.id + '" data-type="' + loan.type + '">';
                html += '  <div class="budgetura-card-header">';
                html += '    <div>';
                html += '      <h3 class="budgetura-card-name">' + escapeHtml(loan.name) + '</h3>';
                html += '      <span class="budgetura-card-badge">' + loan.type + '</span>';
                html += '    </div>';
                html += '    <div class="budgetura-card-actions">';
                html += '      <button class="budgetura-btn budgetura-btn-small" onclick="window.location=\'?action=edit&edit=' + loan.id + '\'">' + budgeturaL10n.edit + '</button>';
                html += '      <button class="budgetura-btn budgetura-btn-small budgetura-btn-danger budgetura-delete-loan" data-id="' + loan.id + '">' + budgeturaL10n.delete + '</button>';
                html += '    </div>';
                html += '  </div>';
                ...
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Balance</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(loan.balance) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Monthly Payment</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(loan.monthly_payment + loan.extra_payment) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Interest Rate</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatPercentage(loan.interest_rate) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Payoff Date</div>';
                html += '      <div class="budgetura-card-detail-value">' + escapeHtml(loan.payoff_date) + '</div>';
                html += '    </div>';
                html += '  </div>';
        ...
        function initLoanForm() {
            const $form = $('#budgetura-loan-form');
            if (!$form.length) return;
            ...
            $('#calculate-loan-payment').on('click', function() {
                ...
                if (principal > 0 && termMonths > 0) {
                    const payment = BudgeturaCalculator.calculateLoanPayment(principal, rate, termMonths);
        ...
        function calculateLoanPayoffPreview() {
            ...
            if (balance > 0 && totalPayment > 0) {
                ...
                $('#preview-interest').text(BudgeturaCalculator.formatCurrency(totalInterest));
                $('#preview-date').text(payoffDate);
                $('#preview-total').text(BudgeturaCalculator.formatCurrency(totalPaid));

                $('#budgetura-loan-payoff-preview').slideDown();
            } else {
                $('#budgetura-loan-payoff-preview').slideUp();
            }
        }
        ...
        function saveLoan() {
            ...
            $submitBtn.prop('disabled', true).text(budgeturaL10n.saving);

            const method = postId > 0 ? 'PUT' : 'POST';
            const url = budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_loan' + (postId > 0 ? '/' + postId : '');
        ...
        $(document).on('click', '.budgetura-delete-loan', function(e) {
            e.preventDefault();
            if (!confirm(budgeturaL10n.confirm_delete)) return;

            const loanId = $(this).data('id');
            const $loanItem = $(this).closest('.budgetura-card-item');

            $.ajax({
                url: budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_loan/' + loanId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function initBillsManager() {
            if ($('.budgetura-bills-manager').length) {
                loadBillsList();
                initBillForm();
                initBillControls();
            }
        ...
        function loadBillsList() {
            const $list = $('#budgetura-bills-list');
            if (!$list.length) return;

            $.ajax({
                url: budgetura.restUrl + 'bills',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
                ...
                error: function(xhr, status, error) {
                    ...
                    $list.html('<div class="budgetura-message error">Failed to load bills</div>');
                }
            });
        }
        ...
        function renderBillsList(bills) {
            ...
            bills.forEach(function(bill) {
                html += '<div class="budgetura-card-item" data-id="' + bill.id + '" data-category="' + bill.category + '" data-essential="' + bill.is_essential + '">';
                html += '  <div class="budgetura-card-header">';
                html += '    <div>';
                html += '      <h3 class="budgetura-card-name">' + escapeHtml(bill.name) + '</h3>';
                html += '      <span class="budgetura-card-badge">' + bill.category + '</span>';
                if (bill.is_essential == 1) {
                    html += '      <span class="budgetura-card-badge success">Essential</span>';
                }
                if (bill.auto_pay == 1) {
                    html += '      <span class="budgetura-card-badge">Auto-Pay</span>';
                }
                html += '    </div>';
                html += '    <div class="budgetura-card-actions">';
                html += '      <button class="budgetura-btn budgetura-btn-small" onclick="window.location=\'?action=edit&edit=' + bill.id + '\'">' + budgeturaL10n.edit + '</button>';
                html += '      <button class="budgetura-btn budgetura-btn-small budgetura-btn-danger budgetura-delete-bill" data-id="' + bill.id + '">' + budgeturaL10n.delete + '</button>';
                html += '    </div>';
                html += '  </div>';

                html += '  <div class="budgetura-card-details">';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Amount</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(bill.amount) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Frequency</div>';
                html += '      <div class="budgetura-card-detail-value">' + bill.frequency + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Monthly Equivalent</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(bill.monthly_equivalent) + '</div>';
                html += '    </div>';
                if (bill.due_date) {
                    html += '    <div class="budgetura-card-detail">';
                    html += '      <div class="budgetura-card-detail-label">Due Date</div>';
                    html += '      <div class="budgetura-card-detail-value">' + bill.due_date + '</div>';
                    html += '    </div>';
                }
                html += '  </div>';
                html += '</div>';
        ...
        function initBillForm() {
            const $form = $('#budgetura-bill-form');
            if (!$form.length) return;
        ...
        function calculateBillPreview() {
            ...
            if (amount > 0) {
                const monthly = BudgeturaCalculator.convertToMonthly(amount, frequency);
                ...
                $('#budgetura-bill-preview').slideDown();
            } else {
                $('#budgetura-bill-preview').slideUp();
            }
        }
        ...
        function saveBill() {
            ...
            const method = postId > 0 ? 'PUT' : 'POST';
            const url = budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_bill' + (postId > 0 ? '/' + postId : '');
        ...
        $(document).on('click', '.budgetura-delete-bill', function(e) {
            e.preventDefault();
            if (!confirm(budgeturaL10n.confirm_delete)) return;

            const billId = $(this).data('id');
            const $billItem = $(this).closest('.budgetura-card-item');

            $.ajax({
                url: budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_bill/' + billId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function initGoalsManager() {
            if ($('.budgetura-goals-manager').length) {
                loadGoalsList();
                initGoalForm();
                initGoalControls();
            }
        ...
        function loadGoalsList() {
            const $list = $('#budgetura-goals-list');
            if (!$list.length) return;

            $.ajax({
                url: budgetura.restUrl + 'goals',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
                ...
                error: function(xhr, status, error) {
                    ...
                    $list.html('<div class="budgetura-message error">Failed to load goals</div>');
                }
            });
        }
        ...
        function renderGoalsList(goals) {
            ...
            goals.forEach(function(goal) {
                ...
                html += '<div class="budgetura-card-item" data-id="' + goal.id + '" data-type="' + goal.type + '" data-priority="' + goal.priority + '">';
                html += '  <div class="budgetura-card-header">';
                html += '    <div>';
                html += '      <h3 class="budgetura-card-name">' + escapeHtml(goal.name) + '</h3>';
                html += '      <span class="budgetura-card-badge">' + goal.type.replace('_', ' ') + '</span>';
                html += '      <span class="budgetura-card-badge ' + goal.priority + '">' + goal.priority + '</span>';
                html += '    </div>';
                html += '    <div class="budgetura-card-actions">';
                html += '      <button class="budgetura-btn budgetura-btn-small" onclick="window.location=\'?action=edit&edit=' + goal.id + '\'">' + budgeturaL10n.edit + '</button>';
                html += '      <button class="budgetura-btn budgetura-btn-small budgetura-btn-danger budgetura-delete-goal" data-id="' + goal.id + '">' + budgeturaL10n.delete + '</button>';
                html += '    </div>';
                html += '  </div>';

                html += '  <div class="budgetura-progress" style="height: 25px; margin: 15px 0;">';
                html += '    <div class="budgetura-progress-bar ' + progressClass + '" style="width: ' + Math.min(goal.progress_percentage, 100) + '%"></div>';
                html += '  </div>';

                html += '  <div class="budgetura-card-details">';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Progress</div>';
                html += '      <div class="budgetura-card-detail-value ' + progressClass + '">' + formatPercentage(goal.progress_percentage) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Current / Target</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(goal.current_amount) + ' / ' + formatCurrency(goal.target_amount) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Monthly Contribution</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(goal.monthly_contribution) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Months to Goal</div>';
                html += '      <div class="budgetura-card-detail-value">' + (goal.months_to_goal === Infinity ? '∞' : goal.months_to_goal) + '</div>';
                html += '    </div>';
                html += '  </div>';
                html += '</div>';
            });
        ...
        function initGoalForm() {
            const $form = $('#budgetura-goal-form');
            if (!$form.length) return;
        ...
        function calculateGoalPreview() {
            ...
            if (targetAmount > 0) {
                ...
                const progress = BudgeturaCalculator.calculateProgress(currentAmount, targetAmount);
                const monthsToGoal = BudgeturaCalculator.calculateMonthsToGoal(currentAmount, targetAmount, monthlyContribution);
                ...
                $('#budgetura-goal-preview').slideDown();
            } else {
                $('#budgetura-goal-preview').slideUp();
            }
        }
        ...
        function saveGoal() {
            ...
            const method = postId > 0 ? 'PUT' : 'POST';
            const url = budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_goal' + (postId > 0 ? '/' + postId : '');
        ...
        $(document).on('click', '.budgetura-delete-goal', function(e) {
            e.preventDefault();
            if (!confirm(budgeturaL10n.confirm_delete)) return;

            const goalId = $(this).data('id');
            const $goalItem = $(this).closest('.budgetura-card-item');

            $.ajax({
                url: budgetura.restUrl.replace('/budgetura/v1/', '/wp/v2/') + 'dd_goal/' + goalId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function initActionPlanManager() {
            if ($('.budgetura-action-plan').length) {
                initActionPlanForm();
            }
        }
        ...
        function initActionPlanForm() {
            const $form = $('#budgetura-action-plan-form');
            if (!$form.length) return;
            ...
            $('#toggle-schedule').on('click', function() {
                const $details = $('#budgetura-schedule-details');
                const $btn = $(this);
        ...
        function displayPayoffTimeline(plan) {
            ...
            plan.order.forEach(function(debt, index) {
                ...
                html += '<div class="budgetura-timeline-item">';
                html += '  <div class="budgetura-timeline-number">' + (index + 1) + '</div>';
                html += '  <div class="budgetura-timeline-content">';
                html += '    <h4>' + escapeHtml(debt.name) + '</h4>';
                html += '    <div class="budgetura-timeline-details">';
                ...
                html += '    </div>';
                html += '    <div class="budgetura-timeline-payoff">';
            ...
            $('#budgetura-timeline-items').html(html);
        }
        ...
        function showAllPlanSections() {
            $('#budgetura-plan-summary').slideDown();
            $('#budgetura-payoff-timeline').slideDown();
            $('#budgetura-payment-schedule').slideDown();
            $('#budgetura-action-items').slideDown();
            $('#budgetura-plan-actions').slideDown();
        }

        function hideAllPlanSections() {
            $('#budgetura-plan-summary').hide();
            $('#budgetura-payoff-timeline').hide();
            $('#budgetura-payment-schedule').hide();
            $('#budgetura-action-items').hide();
            $('#budgetura-plan-actions').hide();
        ...
        function createSnapshot() {
            if (confirm('Create a snapshot of your current financial state?')) {
                $.ajax({
                    url: budgetura.restUrl + 'snapshot',
                    method: 'POST',
                    beforeSend: function(xhr) {
                        xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                    },
        ...
        function loadSnapshotsList() {
            $.ajax({
                url: budgetura.restUrl + 'snapshots',
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
                success: function(response) {
                    if (response.length === 0) {
                        ...
                        $('#budgetura-snapshot-comparison').hide();
                        $('#budgetura-progress-overview').hide();
                        ...
                        $('#budgetura-snapshot-comparison').show();
                    }
                },
                error: function(xhr, status, error) {
                    console.error('Failed to load snapshots:', error);
                    $('#snapshots-list-container').html('<p class="budgetura-message error">Failed to load snapshots</p>');
                }
            });
        }
        ...
        function renderSnapshotsList(snapshots) {
            let html = '<div class="budgetura-snapshots-timeline">';
            ...
            snapshots.forEach(function(snapshot, index) {
                ...
                html += '<div class="budgetura-snapshot-item ' + (isLatest ? 'latest' : '') + '">';
        ...
        function calculateProgressOverview(snapshots) {
            ...
            $('#budgetura-progress-overview').show();

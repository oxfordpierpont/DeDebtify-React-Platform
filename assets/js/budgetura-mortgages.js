...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function initMortgageManager() {
            if ($('.budgetura-mortgages-manager').length) {
                loadMortgages();
                initMortgageForm();
            }
        }
        ...
        function renderMortgages(mortgages) {
            ...
            mortgages.forEach(function(mortgage) {
                ...
                html += '<div class="budgetura-card-item" data-id="' + mortgage.id + '">';
                html += '  <div class="budgetura-card-header">';
                html += '    <h3 class="budgetura-card-name">' + escapeHtml(mortgage.name) + '</h3>';
                html += '    <div class="budgetura-card-actions">';
                html += '      <button class="budgetura-btn-icon budgetura-btn-edit" data-id="' + mortgage.id + '" title="Edit">';
                html += '        <span class="dashicons dashicons-edit"></span>';
                html += '      </button>';
                html += '      <button class="budgetura-btn-icon budgetura-btn-delete" data-id="' + mortgage.id + '" title="Delete">';
                ...
                html += '      <span class="budgetura-card-detail-label">Current Balance</span>';
                html += '      <span class="budgetura-card-detail-value">' + formatCurrency(balance) + '</span>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <span class="budgetura-card-detail-label">Interest Rate</span>';
                html += '      <span class="budgetura-card-detail-value">' + parseFloat(mortgage.interest_rate).toFixed(2) + '%</span>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <span class="budgetura-card-detail-label">Total Monthly</span>';
                html += '      <span class="budgetura-card-detail-value">' + formatCurrency(totalMonthly) + '</span>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <span class="budgetura-card-detail-label">Years Remaining</span>';
                html += '      <span class="budgetura-card-detail-value">' + yearsRemaining + '</span>';
                html += '    </div>';
                html += '  </div>';
            ...
            $('#budgetura-mortgages-list').html(html).show();

            // Attach event handlers
            $('.budgetura-btn-edit').on('click', function() {
                const id = $(this).data('id');
                window.location.href = '?action=edit&edit=' + id;
            });

            $('.budgetura-btn-delete').on('click', function() {
                const id = $(this).data('id');
                deleteMortgage(id);
            });
        }
        ...
        function initMortgageForm() {
            const $form = $('#budgetura-mortgage-form');
        ...
        function updateMortgagePreview() {
            ...
            if (monthlyPayment === 0 || currentBalance === 0) {
                $('#budgetura-mortgage-payoff-preview').hide();
                return;
            }
            ...
            $('#budgetura-mortgage-payoff-preview').fadeIn();
        }
        ...
        function saveMortgage() {
            ...
            const url = isEdit
                ? budgetura.restUrl + '../wp/v2/dd_mortgage/' + postId
                : budgetura.restUrl + '../wp/v2/dd_mortgage';
        ...
        function deleteMortgage(id) {
            ...
            $.ajax({
                url: budgetura.restUrl + '../wp/v2/dd_mortgage/' + id,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function showMessage(message, type) {
            ...
            $('.budgetura-mortgages-manager').prepend($message);

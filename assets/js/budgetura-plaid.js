/**
 * Budgetura Plaid Integration JavaScript
 *
...
(function($) {
    ...
    $(document).ready(function() {
        if ($('.budgetura-account-sync').length) {
            initAccountSync();
        }
    });
    ...
    function loadLinkedAccounts() {
        $.ajax({
            url: budgetura.restUrl + 'plaid/linked-accounts',
            method: 'GET',
            ...
            error: function(xhr, status, error) {
                ...
                $('#dd-linked-accounts-list').html('<div class="budgetura-message error">Failed to load accounts</div>');
            }
        });
    }
    ...
    function renderLinkedAccounts(accounts) {
        ...
        accounts.forEach(function(account) {
            ...
            html += '    <button class="budgetura-btn budgetura-btn-sm dd-sync-account-btn" data-item-id="' + escapeHtml(account.item_id) + '">';
            html += '      <span class="dashicons dashicons-update"></span> Sync Now';
            html += '    </button>';
            html += '    <button class="budgetura-btn budgetura-btn-sm budgetura-btn-secondary dd-disconnect-account-btn" data-item-id="' + escapeHtml(account.item_id) + '">';
    ...
    function openPlaidLink() {
        ...
        $.ajax({
            url: budgetura.restUrl + 'plaid/create-link-token',
            method: 'POST',
            beforeSend: function(xhr) {
                xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
            },
    ...
    function exchangePublicToken(publicToken) {
        ...
        $.ajax({
            url: budgetura.restUrl + 'plaid/exchange-token',
            method: 'POST',
    ...
    function syncAccounts() {
        ...
        $.ajax({
            url: budgetura.restUrl + 'plaid/sync',
            method: 'POST',
            beforeSend: function(xhr) {
                xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
            },
    ...
    function disconnectAccount(itemId) {
        ...
        $.ajax({
            url: budgetura.restUrl + 'plaid/disconnect',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                item_id: itemId

...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function loadDashboardData() {
            showLoading($('#budgetura-dashboard-stats'));

            $.ajax({
                url: budgetura.restUrl + 'dashboard',
                method: 'GET',
                ...
                complete: function() {
                    hideLoading($('#budgetura-dashboard-stats'));
                }
            });
        }
        ...
        function calculateDebtFreeDate() {
            Promise.all([
                ...
                $.ajax({
                    url: budgetura.restUrl + 'mortgages',
                    method: 'GET',
                    beforeSend: function(xhr) {
                        xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                    }
        ...
        function loadCreditCards() {
            const $container = $('#budgetura-credit-cards');
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    $container.html('<p class="budgetura-message error">Failed to load credit cards</p>');
                },
                complete: function() {
        ...
        function renderCreditCards(cards, $container) {
            ...
            let html = '';
            cards.forEach(function(card) {
                html += '<div class="budgetura-card-item">';
                html += '  <div class="budgetura-card-header">';
                html += '    <h3 class="budgetura-card-name">' + escapeHtml(card.name) + '</h3>';
                html += '    <span class="budgetura-card-badge ' + card.status + '">' + card.status.replace('_', ' ') + '</span>';
                html += '  </div>';
                ...
                html += '      <div class="budgetura-card-detail-value">' + formatPercentage(card.utilization) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                ...
                html += '    <div class="budgetura-progress-bar ' + getUtilizationClass(card.utilization) + '" style="width: ' + card.utilization + '%"></div>';
                html += '  </div>';
         ...
         */
        function loadLoans() {
            const $container = $('#budgetura-loans');
            if (!$container.length) return;
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    $container.html('<p class="budgetura-message error">Failed to load loans</p>');
                },
                complete: function() {
                    hideLoading($container);
                }
        ...
        function renderLoans(loans, $container) {
            ...
            let html = '<div class="budgetura-card-list">';
            loans.forEach(function(loan) {
                html += '<div class="budgetura-card-item">';
                html += '  <div class="budgetura-card-header">';
                ...
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Payment</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatCurrency(loan.monthly_payment) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Rate</div>';
                html += '      <div class="budgetura-card-detail-value">' + formatPercentage(loan.interest_rate) + '</div>';
                html += '    </div>';
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Payoff</div>';
         ...
         */
        function loadBills() {
            const $container = $('#budgetura-bills');
            if (!$container.length) return;
         ...
         */
        function loadGoals() {
            const $container = $('#budgetura-goals');
            if (!$container.length) return;
        ...
        function renderGoals(goals, $container) {
            ...
            goals.forEach(function(goal) {
                ...
                html += '  <div class="budgetura-progress">';
                html += '    <div class="budgetura-progress-bar success" style="width: ' + goal.progress_percentage + '%"></div>';
                html += '  </div>';
         ...
         * Create snapshot
         */
        $(document).on('click', '.budgetura-create-snapshot', function(e) {
            e.preventDefault();
        ...
        function showLoading($element) {
            $element.html('<div class="budgetura-loading"><div class="budgetura-spinner"></div></div>');
        }

        function hideLoading($element) {
            $element.find('.budgetura-loading').remove();
        }
        ...
        function applyCreditUtilizationColor(util) {
            const $card = $('#dd-credit-utilization').closest('.budgetura-stat-card');
            $card.removeClass('success warning danger');
            $card.addClass(getUtilizationClass(util));
        }

        function applyDTIColor(dti) {
            const $card = $('#dd-dti-ratio').closest('.budgetura-stat-card');
            $card.removeClass('success warning danger');
        ...
        function loadCreditCardsList() {
            const $container = $('#budgetura-credit-cards-list');
            if (!$container.length) return;
        ...
        function renderCreditCardItem(card) {
            ...
            let html = '<div class="budgetura-card-item" data-card-id="' + card.id + '">';

            // Header
            html += '  <div class="budgetura-card-header">';
            html += '    <h3 class="budgetura-card-name">' + escapeHtml(card.name) + '</h3>';
            html += '    <div class="budgetura-card-actions">';
            html += '      <span class="budgetura-badge budgetura-badge-' + statusBadgeClass + '">' + card.status.replace('_', ' ').toUpperCase() + '</span>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-edit" data-card-id="' + card.id + '" title="Edit"><span class="dashicons dashicons-edit"></span></button>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-delete" data-card-id="' + card.id + '" title="Delete"><span class="dashicons dashicons-trash"></span></button>';
            html += '    </div>';
            html += '  </div>';
            ...
            html += '      <div class="budgetura-card-detail-value">' + formatCurrency((parseFloat(card.minimum_payment) || 0) + (parseFloat(card.extra_payment) || 0)) + '</div>';
            html += '    </div>';
            html += '  </div>';

            // Utilization progress bar
            html += '  <div class="budgetura-card-utilization">';
            html += '    <div class="budgetura-card-utilization-label">Utilization: ' + formatPercentage(utilization) + '</div>';
            html += '    <div class="budgetura-progress">';
            html += '      <div class="budgetura-progress-bar budgetura-progress-' + utilizationClass + '" style="width: ' + Math.min(utilization, 100) + '%"></div>';
            html += '    </div>';
            html += '  </div>';
            ...
            if (card.status === 'active' && parseFloat(card.balance) > 0) {
                ...
                html += '    <div class="budgetura-payoff-scenarios">';

                // Scenario 1: Minimum only
                if (card.payoff_minimum) {
                    html += '      <div class="budgetura-payoff-scenario">';
                ...
                if (card.payoff_minimum && card.payoff_with_extra) {
                    ...
                    if (savings > 0) {
                        html += '      <div class="budgetura-payoff-savings">';
        ...
        function loadCreditCardData(postId) {
            $.ajax({
                url: budgetura.restUrl + 'credit-cards/' + postId,
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function saveCreditCard() {
            ...
            const url = isEdit ? budgetura.restUrl + 'credit-cards/' + postId : budgetura.restUrl + 'credit-cards';
         ...
         */
        $(document).on('click', '.budgetura-btn-edit', function(e) {
            e.preventDefault();
         ...
         */
        $(document).on('click', '.budgetura-btn-delete', function(e) {
            e.preventDefault();

            const cardId = $(this).data('card-id');
            const $cardItem = $(this).closest('.budgetura-card-item');
            const cardName = $cardItem.find('.budgetura-card-name').text();
            ...
            $.ajax({
                url: budgetura.restUrl + 'credit-cards/' + cardId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function loadLoansList() {
            const $container = $('#budgetura-loans-list');
            if (!$container.length) return;
        ...
        function renderLoanItem(loan) {
            ...
            let html = '<div class="budgetura-card-item" data-loan-id="' + loan.id + '">';

            // Header
            html += '  <div class="budgetura-card-header">';
            html += '    <h3 class="budgetura-card-name">' + escapeHtml(loan.name) + '</h3>';
            html += '    <div class="budgetura-card-actions">';
            html += '      <span class="budgetura-badge budgetura-badge-info">' + loanTypeLabel + '</span>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-edit-loan" data-loan-id="' + loan.id + '" title="Edit"><span class="dashicons dashicons-edit"></span></button>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-delete-loan" data-loan-id="' + loan.id + '" title="Delete"><span class="dashicons dashicons-trash"></span></button>';
            html += '    </div>';
            html += '  </div>';
            ...
            html += '  <div class="budgetura-card-details">';
            html += '    <div class="budgetura-card-detail">';
            html += '      <div class="budgetura-card-detail-label">Current Balance</div>';
            html += '      <div class="budgetura-card-detail-value">' + formatCurrency(loan.current_balance) + '</div>';
            html += '    </div>';
            html += '    <div class="budgetura-card-detail">';
            html += '      <div class="budgetura-card-detail-label">Monthly Payment</div>';
            html += '      <div class="budgetura-card-detail-value">' + formatCurrency(loan.monthly_payment) + '</div>';
            ...
            html += '      <div class="budgetura-card-detail-value" style="font-size: 1.1rem;">' + (loan.payoff_date || '—') + '</div>';
            html += '    </div>';
            html += '  </div>';
        ...
        function loadLoanData(postId) {
            $.ajax({
                url: budgetura.restUrl + 'loans/' + postId,
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function updateLoanPayoffPreview() {
            ...
            if (balance <= 0 || interestRate <= 0 || monthlyPayment <= 0) {
                $('#budgetura-loan-payoff-preview').hide();
                return;
            }
            ...
            $('#budgetura-loan-payoff-preview').show();
        }
        ...
        function saveLoan() {
            ...
            const url = isEdit ? budgetura.restUrl + 'loans/' + postId : budgetura.restUrl + 'loans';
            const method = isEdit ? 'PUT' : 'POST';
        ...
        $(document).on('click', '.budgetura-btn-edit-loan', function(e) {
            e.preventDefault();
            const loanId = $(this).data('loan-id');
            window.location.href = window.location.pathname + '?action=edit&edit=' + loanId;
        });

        $(document).on('click', '.budgetura-btn-delete-loan', function(e) {
            e.preventDefault();
            const loanId = $(this).data('loan-id');
            const $loanItem = $(this).closest('.budgetura-card-item');
            const loanName = $loanItem.find('.budgetura-card-name').text();
            ...
            $.ajax({
                url: budgetura.restUrl + 'loans/' + loanId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function loadBillsList() {
            const $container = $('#budgetura-bills-list');
            if (!$container.length) return;
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error('Failed to load bills:', error);
                    $container.html('<p class="budgetura-message error">Failed to load bills</p>');
                },
                complete: function() {
                    hideLoading($container);
                }
        ...
        function renderBillItem(bill) {
            ...
            let html = '<div class="budgetura-card-item" data-bill-id="' + bill.id + '">';

            html += '  <div class="budgetura-card-header">';
            html += '    <h3 class="budgetura-card-name">' + escapeHtml(bill.name) + '</h3>';
            html += '    <div class="budgetura-card-actions">';
            html += '      <span class="budgetura-badge budgetura-badge-info">' + categoryLabel + '</span>';
            if (isEssential) {
                html += '      <span class="budgetura-badge budgetura-badge-success">Essential</span>';
            }
            html += '      <button class="budgetura-btn-icon budgetura-btn-edit-bill" data-bill-id="' + bill.id + '" title="Edit"><span class="dashicons dashicons-edit"></span></button>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-delete-bill" data-bill-id="' + bill.id + '" title="Delete"><span class="dashicons dashicons-trash"></span></button>';
            html += '    </div>';
            html += '  </div>';
            ...
            html += '      <div class="budgetura-card-detail-value" style="font-size: 1.1rem;">' + bill.frequency + '</div>';
            html += '    </div>';
            html += '    <div class="budgetura-card-detail">';
            ...
            if (bill.due_date) {
                ...
                html += '      <div class="budgetura-card-detail-value" style="font-size: 1.1rem;">Day ' + bill.due_date + '</div>';
                html += '    </div>';
            }
            html += '  </div>';
        ...
        function loadBillData(postId) {
            $.ajax({
                url: budgetura.restUrl + 'bills/' + postId,
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function saveBill() {
            ...
            const url = isEdit ? budgetura.restUrl + 'bills/' + postId : budgetura.restUrl + 'bills';
            const method = isEdit ? 'PUT' : 'POST';
        ...
        $(document).on('click', '.budgetura-btn-edit-bill', function(e) {
            e.preventDefault();
            const billId = $(this).data('bill-id');
            window.location.href = window.location.pathname + '?action=edit&edit=' + billId;
        });

        $(document).on('click', '.budgetura-btn-delete-bill', function(e) {
            e.preventDefault();
            const billId = $(this).data('bill-id');
            const $billItem = $(this).closest('.budgetura-card-item');
            const billName = $billItem.find('.budgetura-card-name').text();
            ...
            $.ajax({
                url: budgetura.restUrl + 'bills/' + billId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function loadGoalsList() {
            const $container = $('#budgetura-goals-list');
            if (!$container.length) return;
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error('Failed to load goals:', error);
                    $container.html('<p class="budgetura-message error">Failed to load goals</p>');
                },
                complete: function() {
                    hideLoading($container);
                }
        ...
        function renderGoalItem(goal) {
            ...
            let html = '<div class="budgetura-card-item" data-goal-id="' + goal.id + '">';

            html += '  <div class="budgetura-card-header">';
            html += '    <h3 class="budgetura-card-name">' + escapeHtml(goal.name) + '</h3>';
            html += '    <div class="budgetura-card-actions">';
            html += '      <span class="budgetura-badge budgetura-badge-' + priorityClass + '">' + goal.priority.toUpperCase() + '</span>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-edit-goal" data-goal-id="' + goal.id + '" title="Edit"><span class="dashicons dashicons-edit"></span></button>';
            html += '      <button class="budgetura-btn-icon budgetura-btn-delete-goal" data-goal-id="' + goal.id + '" title="Delete"><span class="dashicons dashicons-trash"></span></button>';
            html += '    </div>';
            html += '  </div>';

            html += '  <div class="budgetura-goal-progress-display">';
            html += '    <div class="budgetura-progress" style="height: 25px; margin-bottom: 10px;">';
            html += '      <div class="budgetura-progress-bar budgetura-progress-success" style="width: ' + Math.min(progress, 100) + '%"></div>';
            html += '    </div>';
            ...
            html += '  <div class="budgetura-card-details">';
            html += '    <div class="budgetura-card-detail">';
            html += '      <div class="budgetura-card-detail-label">Current</div>';
            html += '      <div class="budgetura-card-detail-value">' + formatCurrency(goal.current_amount) + '</div>';
            html += '    </div>';
            html += '    <div class="budgetura-card-detail">';
            html += '      <div class="budgetura-card-detail-label">Target</div>';
            html += '      <div class="budgetura-card-detail-value">' + formatCurrency(goal.target_amount) + '</div>';
            html += '    </div>';
            html += '    <div class="budgetura-card-detail">';
            html += '      <div class="budgetura-card-detail-label">Remaining</div>';
            html += '      <div class="budgetura-card-detail-value">' + formatCurrency(goal.remaining_amount) + '</div>';
            html += '    </div>';
            if (goal.monthly_contribution > 0) {
                html += '    <div class="budgetura-card-detail">';
                html += '      <div class="budgetura-card-detail-label">Months to Goal</div>';
                html += '      <div class="budgetura-card-detail-value" style="font-size: 1.1rem;">' + (goal.months_to_goal || '—') + '</div>';
                html += '    </div>';
            }
            html += '  </div>';
        ...
        function loadGoalData(postId) {
            $.ajax({
                url: budgetura.restUrl + 'goals/' + postId,
                method: 'GET',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },
        ...
        function saveGoal() {
            ...
            const url = isEdit ? budgetura.restUrl + 'goals/' + postId : budgetura.restUrl + 'goals';
            const method = isEdit ? 'PUT' : 'POST';
        ...
        $(document).on('click', '.budgetura-btn-edit-goal', function(e) {
            e.preventDefault();
            const goalId = $(this).data('goal-id');
            window.location.href = window.location.pathname + '?action=edit&edit=' + goalId;
        });

        $(document).on('click', '.budgetura-btn-delete-goal', function(e) {
            e.preventDefault();
            const goalId = $(this).data('goal-id');
            const $goalItem = $(this).closest('.budgetura-card-item');
            const goalName = $goalItem.find('.budgetura-card-name').text();
            ...
            $.ajax({
                url: budgetura.restUrl + 'goals/' + goalId,
                method: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-WP-Nonce', budgetura.restNonce);
                },

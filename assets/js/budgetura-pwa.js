...
(function($) {
    ...
    function registerServiceWorker() {
        navigator.serviceWorker.register(budgeturaPWA.serviceWorkerUrl, {
            scope: '/'
        })
        .then(function(registration) {
            console.log('[Budgetura PWA] Service Worker registered:', registration.scope);
            ...
            if (budgeturaPWA.pushEnabled && registration.pushManager) {
                setupPushNotifications(registration);
            }
        })
        .catch(function(error) {
            console.error('[Budgetura PWA] Service Worker registration failed:', error);
        });
    }
    ...
    function setupInstallPrompt() {
        window.addEventListener('beforeinstallprompt', function(e) {
            console.log('[Budgetura PWA] Install prompt available');
        ...
        window.addEventListener('appinstalled', function() {
            console.log('[Budgetura PWA] App installed successfully');
            deferredPrompt = null;
            hideInstallButton();
                ...
                <div id="dd-pwa-install-banner" class="dd-pwa-banner">
                    <div class="dd-pwa-banner-content">
                        ...
                        <div class="dd-pwa-banner-text">
                            <strong>Install Budgetura</strong>
                            <p>Access your finances faster with our app</p>
                        </div>
                        <div class="dd-pwa-banner-actions">
                            <button id="dd-pwa-install-btn" class="budgetura-btn budgetura-btn-primary">
                                Install
                            </button>
                            <button id="dd-pwa-dismiss-btn" class="budgetura-btn budgetura-btn-secondary">
                                Later
                            </button>
                        </div>
                    </div>
    ...
    function installApp() {
        ...
        deferredPrompt.userChoice.then(function(choiceResult) {
            if (choiceResult.outcome === 'accepted') {
                console.log('[Budgetura PWA] User accepted install');
            } else {
                console.log('[Budgetura PWA] User dismissed install');
                localStorage.setItem('dd_pwa_install_dismissed', Date.now());
            }
    ...
    function setupPushNotifications(registration) {
        ...
        if (!('Notification' in window) || !('PushManager' in window)) {
            console.log('[Budgetura PWA] Push notifications not supported');
            return;
        }
    ...
    function showNotificationPrompt() {
        ...
        if ($('#dd-pwa-notification-prompt').length === 0 && !localStorage.getItem('dd_notification_dismissed')) {
            const prompt = `
                <div id="dd-pwa-notification-prompt" class="dd-pwa-notification-prompt">
                    <div class="dd-notification-prompt-content">
                        ...
                        <button id="dd-enable-notifications-btn" class="budgetura-btn budgetura-btn-small">
                            Enable
                        </button>
            ...
            $('.budgetura-dashboard').prepend(prompt);
    ...
    function requestNotificationPermission() {
        Notification.requestPermission().then(function(permission) {
            console.log('[Budgetura PWA] Notification permission:', permission);
    ...
    function subscribeToPush(registration) {
        const vapidPublicKey = budgeturaPWA.vapidPublicKey;

        if (!vapidPublicKey) {
            console.log('[Budgetura PWA] VAPID public key not configured');
            return;
        }
        ...
        .then(function(subscription) {
            console.log('[Budgetura PWA] Push subscription:', subscription);

            // Send subscription to server
            return fetch(budgeturaPWA.ajaxUrl, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    action: 'budgetura_save_push_subscription',
                    subscription: subscription,
                    nonce: budgeturaPWA.nonce
                })
            });
        })
        .then(function(response) {
            if (response.ok) {
                console.log('[Budgetura PWA] Subscription saved to server');
            }
        })
        .catch(function(error) {
            console.error('[Budgetura PWA] Push subscription failed:', error);
        });
    }
    ...
    function setupConnectionMonitoring() {
        window.addEventListener('online', function() {
            console.log('[Budgetura PWA] Connection restored');
            ...
            if (swRegistration && swRegistration.sync) {
                swRegistration.sync.register('sync-data').catch(function(error) {
                    console.error('[Budgetura PWA] Sync registration failed:', error);
                });
            }
        });

        window.addEventListener('offline', function() {
            console.log('[Budgetura PWA] Connection lost');
        ...
        const updateBanner = `
            <div id="dd-pwa-update-banner" class="dd-pwa-update-banner">
                <div class="dd-update-banner-content">
                    ...
                    <button id="dd-pwa-update-btn" class="budgetura-btn budgetura-btn-small">
                        Update Now
                    </button>
                </div>
            </div>
    ...
    window.createSnapshotOffline = function(data) {
        if (!navigator.onLine && swRegistration && swRegistration.sync) {
            ...
            caches.open('budgetura-v1.0.0-sync').then(function(cache) {

...
class Budgetura_AI_Coach {
    ...
    public function __construct() {
        ...
        $this->api_key = get_option( 'budgetura_ai_api_key', '' );
        $provider = get_option( 'budgetura_ai_provider', 'openai' );

        // Set provider-specific settings
        switch ( $provider ) {
            case 'anthropic':
                $this->api_endpoint = 'https://api.anthropic.com/v1/messages';
                $this->model = get_option( 'budgetura_ai_model', 'claude-3-5-sonnet-20241022' );
                break;

            case 'openrouter':
                $this->api_endpoint = 'https://openrouter.ai/api/v1/chat/completions';
                $this->model = get_option( 'budgetura_ai_model', 'anthropic/claude-3.5-sonnet' );
                break;

            case 'openai':
            default:
                $this->api_endpoint = 'https://api.openai.com/v1/chat/completions';
                $this->model = get_option( 'budgetura_ai_model', 'gpt-4o' );
                break;
        }
    ...
    private function get_system_prompt() {
        return "You are an expert AI Financial Coach integrated into Budgetura, a debt management WordPress plugin. Your role is to provide personalized, empathetic, and actionable financial advice to users.
...
- Keep responses conversational but professional";
    ...
    private function call_openai_api( $messages ) {
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'Budgetura AI Coach Error: ' . $response->get_error_message() );
        ...
        if ( isset( $body['error'] ) ) {
            error_log( 'Budgetura AI Coach API Error: ' . json_encode( $body['error'] ) );
    ...
    private function call_openrouter_api( $messages ) {
        ...
        $response = wp_remote_post( $this->api_endpoint, array(
            'headers' => array(
                ...
                'X-Title' => $site_name . ' - Budgetura AI Coach'
            ),
            'body' => json_encode( $request_body ),
            'timeout' => 60
        ) );
    ...
    private function get_setup_message() {
        $settings_url = admin_url( 'admin.php?page=budgetura-settings#ai-coach' );

        return "👋 Welcome to Budgetura AI Coach!\n\n" .
               "To start using the AI Financial Coach, you'll need to configure your AI provider settings.\n\n" .
               "**Setup Steps:**\n" .
               "1. Go to Budgetura Settings > AI Coach\n" .

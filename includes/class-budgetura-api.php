<?php
/**
 * REST API endpoints for Budgetura.
 *
...
class Budgetura_API {
    ...
    public function get_dashboard_data( $request ) {
        ...
        $data = array(
            'total_debt' => Budgetura_Calculations::get_total_debt( $user_id ),
            'credit_card_debt' => Budgetura_Calculations::get_total_credit_card_debt( $user_id ),
            'loan_debt' => Budgetura_Calculations::get_total_loan_debt( $user_id ),
            'mortgage_debt' => Budgetura_Calculations::get_total_mortgage_debt( $user_id ),
            'monthly_payments' => Budgetura_Calculations::get_total_monthly_payments( $user_id ),
            'monthly_bills' => Budgetura_Calculations::get_total_monthly_bills( $user_id ),
            'dti_ratio' => Budgetura_Calculations::get_user_dti( $user_id ),
            'credit_utilization' => Budgetura_Calculations::get_overall_credit_utilization( $user_id ),
    ...
    public function create_snapshot( $request ) {
        $user_id = get_current_user_id();
        $post_id = Budgetura_Calculations::create_snapshot( $user_id );

        if ( is_wp_error( $post_id ) ) {
            return new WP_Error(
                'snapshot_creation_failed',
                __( 'Failed to create snapshot', 'budgetura' ),
                array( 'status' => 500 )
            );
        }
        ...
        return rest_ensure_response( array(
            'success' => true,
            'message' => __( 'Snapshot created successfully', 'budgetura' ),
            'post_id' => $post_id,
        ));
    }
    ...
    public function create_snapshot_with_data( $request ) {
        ...
        if ( empty( $params ) ) {
            return new WP_Error(
                'missing_data',
                __( 'Snapshot data is required', 'budgetura' ),
                array( 'status' => 400 )
            );
        }
    ...
    public function calculate_payoff( $request ) {
        ...
        $months = Budgetura_Calculations::calculate_months_to_payoff( $balance, $interest_rate, $monthly_payment );
        $total_interest = Budgetura_Calculations::calculate_total_interest( $balance, $monthly_payment, $months );
        $payoff_date = Budgetura_Calculations::calculate_payoff_date( $months );
    ...
    public function get_payoff_order( $request ) {
        ...
        if ( $method === 'avalanche' ) {
            $order = Budgetura_Calculations::get_avalanche_order( $user_id );
        } else {
            $order = Budgetura_Calculations::get_snowball_order( $user_id );
        }
    ...
    public function get_user_statistics( $request ) {
        ...
        $credit_cards = Budgetura_Calculations::get_user_credit_cards( $user_id );
        $loans = Budgetura_Calculations::get_user_loans( $user_id );
        $bills = Budgetura_Calculations::get_user_bills( $user_id );
        $goals = Budgetura_Calculations::get_user_goals( $user_id );
        ...
        $data = array(
            ...
            'has_mortgage' => Budgetura_Calculations::get_user_mortgage( $user_id ) !== null,
        );
    ...
    public function get_credit_cards( $request ) {
        $user_id = get_current_user_id();
        $cards = Budgetura_Calculations::get_user_credit_cards( $user_id );
        $cards_data = array();
        ...
        foreach ( $cards as $card ) {
            ...
            $months_to_payoff = Budgetura_Calculations::calculate_months_to_payoff( $balance, $interest_rate, $total_payment );
            $total_interest = Budgetura_Calculations::calculate_total_interest( $balance, $total_payment, $months_to_payoff );
            $utilization = Budgetura_Calculations::calculate_utilization( $balance, $credit_limit );
            ...
            $cards_data[] = array(
                ...
                'total_interest' => $total_interest,
                'payoff_date' => Budgetura_Calculations::calculate_payoff_date( $months_to_payoff ),
            );
        }
    ...
    public function get_bills( $request ) {
        ...
        foreach ( $bills as $bill ) {
            ...
            $monthly_equivalent = Budgetura_Calculations::convert_to_monthly( $amount, $frequency );
    ...
    public function ai_coach_message( $request ) {
        ...
        require_once BUDGETURA_PLUGIN_DIR . 'includes/class-budgetura-ai-coach.php';

        $ai_coach = new Budgetura_AI_Coach();

...
class Budgetura_Plaid {
    ...
    private static function get_api_url() {
        $environment = get_option( 'budgetura_plaid_environment', 'sandbox' );
    ...
    private static function get_credentials() {
        $client_id = get_option( 'budgetura_plaid_client_id', '' );
        $secret = get_option( 'budgetura_plaid_secret', '' );
    ...
    public static function create_link_token( $user_id ) {
        ...
        if ( ! $credentials ) {
            return new WP_Error( 'plaid_not_configured', __( 'Plaid is not configured', 'budgetura' ) );
        }
        ...
        $body = array(
            ...
            'client_name' => get_bloginfo( 'name' ) . ' - Budgetura',
    ...
    public static function sync_user_accounts( $user_id ) {
        ...
        foreach ( $linked_accounts as $index => $account ) {
            ...
            // Sync liabilities to Budgetura
            $synced = self::sync_liabilities_to_budgetura( $user_id, $liabilities_data );
    ...
    /**
     * Sync Plaid liabilities to Budgetura custom post types
     *
     * @param int $user_id User ID
     * @param array $liabilities_data Liabilities data from Plaid
     * @return bool
     */
    private static function sync_liabilities_to_budgetura( $user_id, $liabilities_data ) {

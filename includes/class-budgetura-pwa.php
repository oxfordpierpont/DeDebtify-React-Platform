...
class Budgetura_PWA {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_budgetura_save_push_subscription', array( $this, 'save_push_subscription' ) );
        add_action( 'wp_ajax_budgetura_send_test_notification', array( $this, 'send_test_notification' ) );
    }
    ...
    public function add_pwa_meta_tags() {
        // Only add on Budgetura pages
        if ( ! $this->is_budgetura_page() ) {
            return;
        }
        ...
        echo '<meta name="apple-mobile-web-app-title" content="Budgetura">' . "\n";
        ...
        // Apple touch icons
        echo '<link rel="apple-touch-icon" sizes="180x180" href="' . BUDGETURA_PLUGIN_URL . 'assets/images/icon-192x192.png">' . "\n";
        echo '<link rel="apple-touch-icon" sizes="152x152" href="' . BUDGETURA_PLUGIN_URL . 'assets/images/icon-152x152.png">' . "\n";
        echo '<link rel="apple-touch-icon" sizes="144x144" href="' . BUDGETURA_PLUGIN_URL . 'assets/images/icon-144x144.png">' . "\n";
    }
    ...
    public function add_manifest_link() {
        ...
        $manifest_url = site_url( '/budgetura-manifest.json' );
    ...
    public function register_manifest_route() {
        add_rewrite_rule(
            '^budgetura-manifest\.json$',
            'index.php?budgetura_manifest=1',
            'top'
        );

        add_filter( 'query_vars', function( $vars ) {
            $vars[] = 'budgetura_manifest';
            return $vars;
        });

        add_action( 'template_redirect', function() {
            if ( get_query_var( 'budgetura_manifest' ) ) {
                $this->serve_manifest();
                exit;
            }
        });
    ...
    public function serve_manifest() {
        ...
        $manifest = array(
            'name' => get_bloginfo( 'name' ) . ' - Budgetura',
            'short_name' => 'Budgetura',
            ...
            'icons' => array(
                array(
                    'src' => BUDGETURA_PLUGIN_URL . 'assets/images/icon-192x192.png',
                    'sizes' => '192x192',
                    'type' => 'image/png',
                    'purpose' => 'any maskable'
                ),
                array(
                    'src' => BUDGETURA_PLUGIN_URL . 'assets/images/icon-512x512.png',
                    'sizes' => '512x512',
                    'type' => 'image/png',
                    'purpose' => 'any maskable'
                )
            ...
            'shortcuts' => array(
                array(
                    ...
                    'icons' => array(
                        array(
                            'src' => BUDGETURA_PLUGIN_URL . 'assets/images/icon-96x96.png',
                            'sizes' => '96x96',
                            'type' => 'image/png'
                        )
                    )
    ...
    public function serve_service_worker() {
        ...
        $sw_file = BUDGETURA_PLUGIN_DIR . 'service-worker.js';
    ...
    /**
     * Check if current page is a Budgetura page
     */
    private function is_budgetura_page() {
        global $post;
        ...
        // Check if page uses Budgetura shortcodes
        $budgetura_shortcodes = array(
            'budgetura_dashboard',
            'budgetura_credit_cards',
            'budgetura_loans',
            'budgetura_mortgages',
            'budgetura_bills',
            'budgetura_goals',
            'budgetura_action_plan',
            'budgetura_snapshots',
            'budgetura_ai_coach'
        );

        foreach ( $budgetura_shortcodes as $shortcode ) {
    ...
    public function save_push_subscription() {
        check_ajax_referer( 'budgetura_nonce', 'nonce' );
    ...
    public function send_notification( $user_id, $data ) {
        ...
        do_action( 'budgetura_push_notification_sent', $user_id, $data );
    ...
    public static function schedule_bill_notification( $bill_id ) {
        ...
        if ( $notification_time > time() ) {
            wp_schedule_single_event( $notification_time, 'budgetura_send_bill_reminder', array(
                'user_id' => $user_id,
                'bill_name' => $bill_name,
                'amount' => $amount,
                'due_date' => $due_date
    ...
    public static function send_bill_reminder( $user_id, $bill_name, $amount, $due_date ) {
        ...
        $pwa->send_notification( $user_id, array(
            ...
            'icon' => BUDGETURA_PLUGIN_URL . 'assets/images/icon-192x192.png',
            'badge' => BUDGETURA_PLUGIN_URL . 'assets/images/badge-72x72.png',

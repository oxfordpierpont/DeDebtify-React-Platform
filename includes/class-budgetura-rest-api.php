 ...
 * REST API Class
 *
 * Handles all REST API endpoints for Budgetura
 *
 * @since      1.0.0
 * @package    Budgetura
...
class Budgetura_REST_API {
    ...
    public static function register_routes() {
        ...
        register_rest_route( 'budgetura/v1', '/credit-cards', array(
            'methods' => 'GET',
            'callback' => array( __CLASS__, 'get_credit_cards' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/loans', array(
            'methods' => 'GET',
            'callback' => array( __CLASS__, 'get_loans' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/bills', array(
            'methods' => 'GET',
            'callback' => array( __CLASS__, 'get_bills' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/goals', array(
            'methods' => 'GET',
        ...
        // Plaid endpoints
        register_rest_route( 'budgetura/v1', '/plaid/create-link-token', array(
            'methods' => 'POST',
            'callback' => array( __CLASS__, 'create_link_token' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/plaid/exchange-token', array(
            'methods' => 'POST',
            'callback' => array( __CLASS__, 'exchange_public_token' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/plaid/linked-accounts', array(
            'methods' => 'GET',
            'callback' => array( __CLASS__, 'get_linked_accounts' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/plaid/sync', array(
            'methods' => 'POST',
            'callback' => array( __CLASS__, 'sync_accounts' ),
            'permission_callback' => array( __CLASS__, 'check_user_permission' )
        ) );

        register_rest_route( 'budgetura/v1', '/plaid/disconnect', array(
            'methods' => 'POST',
    ...
    public static function create_link_token( $request ) {
        ...
        if ( ! get_option( 'budgetura_plaid_enabled', 0 ) ) {
            return new WP_Error(
                'plaid_disabled',
                __( 'Plaid integration is not enabled', 'budgetura' ),
                array( 'status' => 403 )
            );
        }

        $user_id = get_current_user_id();
        $result = Budgetura_Plaid::create_link_token( $user_id );
    ...
    public static function exchange_public_token( $request ) {
        ...
        if ( empty( $public_token ) ) {
            return new WP_Error(
                'missing_token',
                __( 'Public token is required', 'budgetura' ),
                array( 'status' => 400 )
            );
        }
        ...
        if ( ! preg_match( '/^public-[a-z0-9-]+$/i', $public_token ) ) {
            return new WP_Error(
                'invalid_token',
                __( 'Invalid token format', 'budgetura' ),
                array( 'status' => 400 )
            );
        }

        $user_id = get_current_user_id();
        $result = Budgetura_Plaid::exchange_public_token( $public_token, $user_id );
        ...
        wp_schedule_single_event( time() + 10, 'budgetura_plaid_initial_sync', array( $user_id ) );

        return new WP_REST_Response( array(
            'success' => true,
            'message' => __( 'Account linked successfully and data is syncing', 'budgetura' )
        ), 200 );
    }
    ...
    public static function get_linked_accounts( $request ) {
        $user_id = get_current_user_id();
        $accounts = Budgetura_Plaid::get_linked_accounts( $user_id );
    ...
    public static function sync_accounts( $request ) {
        $user_id = get_current_user_id();
        $results = Budgetura_Plaid::sync_user_accounts( $user_id );

        return new WP_REST_Response( array(
            'success' => true,
            'message' => sprintf(
                __( 'Synced %d accounts. %d errors occurred.', 'budgetura' ),
                $results['success'],
                $results['errors']
            ),
            'results' => $results
    ...
    public static function disconnect_account( $request ) {
        ...
        if ( empty( $item_id ) ) {
            return new WP_Error(
                'missing_item_id',
                __( 'Item ID is required', 'budgetura' ),
                array( 'status' => 400 )
            );
        }
        ...
        $user_id = get_current_user_id();
        $linked_accounts = Budgetura_Plaid::get_linked_accounts( $user_id );
        $item_exists = false;
        ...
        if ( ! $item_exists ) {
            return new WP_Error(
                'invalid_item',
                __( 'Account not found or you do not have permission to disconnect it', 'budgetura' ),
                array( 'status' => 403 )
            );
        }

        Budgetura_Plaid::disconnect_account( $user_id, $item_id );

        return new WP_REST_Response( array(
            'success' => true,
            'message' => __( 'Account disconnected successfully', 'budgetura' )
        ), 200 );
    }
}

// Register initial sync action
add_action( 'budgetura_plaid_initial_sync', function( $user_id ) {
    Budgetura_Plaid::sync_user_accounts( $user_id );
} );

// Register routes on rest_api_init
add_action( 'rest_api_init', array( 'Budgetura_REST_API', 'register_routes' ) );
